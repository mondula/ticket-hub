...
add_shortcode('thub_documentation', function () {
    ...
    if (!$documentation_enqueue) {
        wp_enqueue_script('thub-documentation-script', PLUGIN_ROOT . 'js/thub-documentation.js', array('jquery'), '', true);
        wp_enqueue_style('thub-documentation-style', PLUGIN_ROOT . 'css/thub-documentation.css', array(), '', 'all');
    ...
    while ($the_query->have_posts()) {
        ...
        echo '<td><div>' . esc_html($document_name) . '</div><a class="thub-button" href="' . esc_url($document_url) . '" target="_blank"' . $download_attribute . '>' . $icon_svg . ' ' . $button_text . '</a></td>';
        echo '</tr>';
    }
    echo '</tbody></table>';
