...
add_shortcode('thub_faqs', function () {
    ...
    if (!$thub_accordion_enqueue) {
        wp_enqueue_script('thub-accordion-script', PLUGIN_ROOT . 'js/thub-accordion.js', array('jquery'), '', true);
        wp_enqueue_style('thub-accordion-style', PLUGIN_ROOT . 'css/thub-accordion.css', array(), '', 'all');
    ...
    if ($the_query->have_posts()) {
        ...
        while ($the_query->have_posts()) {
            ...
            echo apply_filters('the_content', wp_kses_post(get_post_meta(get_the_ID(), '_thub_answer', true)));
            echo '</div>';
            echo '</div>';
        }
