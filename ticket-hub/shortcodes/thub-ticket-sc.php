...
add_shortcode('thub_ticket', function ($atts) {
    ...
    if (!$ticket_enqueue) {
        wp_enqueue_script('thub-lightbox-script', PLUGIN_ROOT . 'js/thub-lightbox.js', array('jquery'), '', true);
        wp_enqueue_style('thub-ticket-style', PLUGIN_ROOT . 'css/thub-ticket.css', array(), '', 'all');
        $ticket_enqueue = true;
    }
...
?>
            <div class="thub-ticket-details">
                ...
                <a href="<?php echo esc_url($tickets_page_url); ?>" class="thub-back-to-archive"><?php _e('Back', 'tickethub') ?></a>
                <?php
                ...
                if (!empty($image_attachments) || !empty($other_attachments)) {
                    ...
                    foreach ($image_attachments as $attachment) {
                        ...
                        if ($image_url) {
                            echo '<a href="' . esc_url($image_url) . '" class="thub-lightbox-trigger"><div class="thub-image-container"><img src="' . esc_url($image_url) . '" alt="' . esc_attr($attachment->post_title) . '" class="thub-ticket-image">' . $zoomSVG . '</div></a>';
                        }
                    }
                    echo '</div>';
...
function thub_display_comment_withub_replies($comment, $depth = 0)
{
    ...
    if ($depth > 0) {
        echo '<div style="margin-left:' . (($depth - 1) * 20) . 'px;" class="thub-vertical-bar"></div>';
    }

    echo '<div class="thub-ticket-comment" style="margin-left:' . ($depth * 30) . 'px;">'; // Indent nested comments
